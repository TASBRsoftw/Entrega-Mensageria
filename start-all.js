...
try {
  execSync('node src/mock-data.js', { stdio: 'inherit' });
} catch (e) {
...
startService('list-service', path.join('src', 'list-service', 'server.js'));

// Iniciar Consumer Log/Notification
startService('consumer-log', path.join('src', 'consumer-log', 'index.js'));

// Iniciar Consumer Analytics
startService('consumer-analytics', path.join('src', 'consumer-analytics', 'index.js'));
